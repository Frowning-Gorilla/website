...
**Dark theme is opt-in, per page, gated on `.auto-theme` — not `.fg`.** `styles.css` has one `@media (prefers-color-scheme: dark)` block; it overrides the surface and accent tokens on `.fg.auto-theme`. Applied to `.fg` it would immediately re-theme the support, privacy and terms pages, none of which has been reviewed in dark. **The two landing pages are opted in** — `shape-n-ship.html` (`<body class="fg theme-shape-n-ship auto-theme">`) and `nearing.html` (`theme-nearing`); no other page is. Add the word page by page as each is reviewed, remove it to force a page permanently light. The frozen Eve pages carry no `.fg` class, so the block can't reach them either way. No toggle — the system setting is already the visitor's answer, and a toggle would need JavaScript.
...
**The "Dark Studio" landing layout — the `.lp-*` block — lives in `styles.css`, shared by both landing pages.** It began as a local `<style>` block on `shape-n-ship.html`; when `nearing.html` adopted the same treatment the block was *promoted* wholesale (not copied), because two copies of a 250-line block in two files is a drift waiting to happen. **Do not copy it back into a page.** If a rule is needed on one landing page and nowhere else, prove that before adding it locally — the two pages drifting apart is exactly what the shared block prevents.
