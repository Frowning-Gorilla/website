...
**Dark theme is opt-in, per page, gated on `.auto-theme` — not `.fg`.** `styles.css` has one `@media (prefers-color-scheme: dark)` block; it overrides the surface and accent tokens on `.fg.auto-theme`. Applied to `.fg` it would immediately re-theme the support, privacy and terms pages, none of which has been reviewed in dark. **`shape-n-ship.html` is the only page opted in** (`<body class="fg theme-shape-n-ship auto-theme">`); add the word page by page as each is reviewed, remove it to force a page permanently light. The frozen Eve pages carry no `.fg` class, so the block can't reach them either way. No toggle — the system setting is already the visitor's answer, and a toggle would need JavaScript.
...
A page may keep a small local `<style>` block only for rules genuinely unique to it (e.g. the homepage's app cards). The Shape n Ship landing keeps its whole "Dark Studio" layout in such a block. **When `nearing.html` adopts the same treatment, promote that block to `styles.css`** rather than copying it into a second page.
...
- **Screenshots are transparent PNGs with baked-in window chrome and a drop shadow** — no CSS frame is added; the window floats on whatever background it sits on, which is why it works in both light and dark. `shape-n-ship-concept-wizard.png` and `shape-n-ship-release-checklist.png` had been exported flattened onto opaque white, which read as a light slab on the light page and a light window-outline on the dark one; both were re-exported with transparency and a shadow matched to the other four. Fix this at the capture step so it doesn't recur.
