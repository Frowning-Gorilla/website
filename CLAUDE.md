...
- **Shape n Ship's model is enumerated in exactly three places**, and nowhere else: the **landing page** (pricing section), its **terms** (callout summary and body), and its **support FAQ**. Each states it in its own register but with **identical substance**:
  - **Free** — your first three projects, each capped at 15 tracked items (bugs and features combined) and one release, from idea through building and testing.
...
| URL | App | Purpose |
|---|---|---|
| `index.html` | — | Company front: intro, app rows, contact |
...
The accent *derivatives* — `--accent-wash`, `--accent-edge`, and the dark-mode `--accent-deep` — are **declared per app**, not once and shared. They were briefly hardcoded to orange (only one page used them), which would have given Nearing orange eyebrows and pill borders on a teal page. They now live under `.fg.theme-shape-n-ship` / `.fg.theme-nearing` (light tints) and `.fg.auto-theme.theme-shape-n-ship` / `.theme-nearing` (dark remaps); company indigo is the default for non-app pages. Don't collapse them back to a single shared declaration.

- **The theme classes work on a nested element, and the accent derivatives carry descendant selectors so they follow.** `index.html` runs on company indigo but puts `theme-shape-n-ship` / `theme-nearing` on each app *row*, not the body. Keyed to `.fg.theme-x` alone (both classes on one element), a nested theme class picked up `--accent` and `--accent-deep` (declared on `.fg.theme-x`) but kept the *outer* scope's `--accent-wash`, `--accent-edge` and dark remaps — so a row's eyebrow got the right colour but its screenshot stage and pill borders stayed indigo. Each derivative block is now written twice, `.fg.theme-x` **and** `.fg .theme-x` (identical specificity; the two forms can never match the same element), so the full set follows the class wherever it sits. `index.html`'s per-row accents depend on this; don't delete the descendant form as a duplicate.
...
**Dark theme is opt-in, per page, gated on `.auto-theme` — not `.fg`.** `styles.css` has one `@media (prefers-color-scheme: dark)` block; it overrides the surface and accent tokens on `.fg.auto-theme`. Applied to `.fg` it would immediately re-theme the support, privacy and terms pages, none of which has been reviewed in dark. **Three pages are opted in** — `shape-n-ship.html` (`theme-shape-n-ship`), `nearing.html` (`theme-nearing`), and `index.html` (`<body class="fg auto-theme">`, **no** theme class — it's the company front, so company indigo, with each app row carrying its own theme class instead). No other page is opted in. Add the word page by page as each is reviewed, remove it to force a page permanently light. The frozen Eve pages carry no `.fg` class, so the block can't reach them either way. No toggle — the system setting is already the visitor's answer, and a toggle would need JavaScript.

- **The accent pair inverts in dark, and the rule is unchanged — don't "correct" it back.** Brand orange `#E0641C` measures 5.5:1 on the dark page, so it passes as text there (it can't on light, at 3.35:1); `--accent-deep` would fail instead (2.4:1), so in dark `--accent-deep` is remapped *upward* to a light tint `#F0A574` (9.5:1). The rule from above still holds — `--accent` for fills, `--accent-deep` for anything read — only the values move. Setting `--accent-deep` back to `#B0470F` in dark would drop it to 2.4:1.
  - **Teal inverts the same way, but converges instead of splitting.** Brand teal `#19BAB0` measures 7.9:1 on the dark page, so it passes as text there (it can't on light, at 2.31:1); its deep light-mode variant `#0E7470` would fail on dark (2.2:1). So for Nearing in dark, `--accent` and `--accent-deep` **converge on the same value**, `#19BAB0` — fills and text both. Same rule, same reason as orange; only the geometry differs.

**The `.lp-band` eyebrow and link colours default to company indigo lifted for dark** (`#A9B4DC`, 10.1:1 on the dark band), with orange scoped to `.fg.theme-shape-n-ship` and teal to `.fg.theme-nearing`. The band is a dark surface in every theme, so its eyebrow always needs the light-on-dark accent regardless of the page's own light/dark. These were once literal orange (only the orange page used a band); once the homepage gained the "About the name" band, that literal orange would have reached a company-indigo page. Same trap as the accent derivatives above — default to indigo, scope the brand colours.

**Self-hosted faces:** DM Sans (body copy, every page) and DM Serif Display (headings on every page *except* the Shape n Ship landing). The landing adds **Archivo** (headlines only) and **JetBrains Mono** (labels and eyebrows — never prose). All four are OFL, latin subset, sitting in `fonts/` with their licences. Never swap any for a `fonts.googleapis.com` link.

A page may keep a small local `<style>` block only for rules genuinely unique to it (e.g. the homepage's app cards).

**The "Dark Studio" landing layout — the `.lp-*` block — lives in `styles.css`, shared by all three main pages: `index.html`, `shape-n-ship.html`, `nearing.html`.** It began as a local `<style>` block on `shape-n-ship.html`; when `nearing.html` adopted the same treatment the block was *promoted* wholesale (not copied), and `index.html` then joined it, because copies of a 250-line block across pages are a drift waiting to happen. **Do not copy it back into a page.** If a rule is needed on one page and nowhere else, prove that before adding it locally — the pages drifting apart is exactly what the shared block prevents. Rules genuinely unique to the homepage (the app rows, the masked logo lockup) sit below the shared block under the **"COMPANY FRONT"** heading, not in a local `<style>` block on the page.

- Every `.lp-*` selector is written `.fg .lp-…` (specificity 0,2,0), deliberately. `styles.css` also styles the bare tags those pages reuse (`.fg header`, `.fg main`, `.fg section`, `.fg footer` — up to 0,1,1), and a single-class `.lp-*` rule (0,1,0) would silently lose to them. The block opens with a small reset that zeroes the inherited tag defaults rather than fighting them rule by rule.
...
- **No app is currently on the App Store.** Eve is delisted, Nearing hasn't shipped, Shape n Ship hasn't launched. **Do not add a store badge or link until a listing is confirmed live** — a badge pointing at nothing is the same bug as the dead Eve link that used to be on the homepage.
  - **The homepage deliberately carries no "Launching soon" pill.** Availability is expressed only by badge presence, and a third status line (on top of the two landing pages') would be a third place to edit on launch day. The homepage has the two badge slots — one per app row — and nothing else. Only the two landing pages carry the pre-launch status line.
- **`index.html` has no local `<style>` block, and the old `.app-card` / `.logo-mark` / `.wordmark` rules are gone.** The homepage was the last page on the old look (indigo hero band, 720px column, serif headings); it now reuses the shared `.lp-*` block like the two landing pages, with its unique rules under "COMPANY FRONT" in `styles.css`. The old app *cards* (small white panels with a 4px accent stripe, showing neither app) are now two large alternating **rows**, each with that app's signature screenshot. The homepage logo mark is a **CSS mask taking `--text`** (like the nav mark), not the bare `frowning-gorilla-logo.svg`: the SVG is a navy illustration and was invisible on the dark page. It loses the gradient, which at 60px reads as flat anyway.
...
- **Screenshots use one of three rules, and which one depends on how the file was exported — not which page it sits on. Don't merge them.**
  - `.shot` — self-framed window captures (transparent PNG with its own window chrome and a baked drop shadow). **Never** framed by CSS; the window floats on whatever background it sits on, which is why it works in both light and dark. This is the Shape n Ship treatment. `shape-n-ship-concept-wizard.png` and `shape-n-ship-release-checklist.png` had been exported flattened onto opaque white, which read as a light slab on the light page and a light window-outline on the dark one; both were re-exported with transparency and a shadow matched to the other four. Fix this at the capture step so it doesn't recur.
  - `.lp-figure` — opaque photographs (JPEG, hard rectangular edge). These *need* the CSS radius, hairline and shadow, because the file has none. Nearing's three landscape captures use this.
  - `.lp-device` — a transparent device cutout (Nearing's iPhone shot). **No border and no radius** — there is no rectangle to trace — and its shadow is `filter: drop-shadow()`, which follows the alpha outline; `box-shadow` would draw the bounding box in mid-air. Adding a border here is the classic mistake.
  - `.lp-app-stage` — a **fourth** context, the exception that proves the other three. On `index.html` the two app rows each sit their screenshot on a tinted stage that frames the capture *for* it, rather than framing each individually. That's what lets one rule hold both kinds side by side on the same page: Shape n Ship's is a transparent `.shot` window capture that must never be framed, Nearing's is an opaque `.lp-figure` JPEG that needs one — the transparent one floats on the tint, the opaque one is contained by the stage. The three-way rule above is unchanged and still applies everywhere else; the stage is specific to the homepage rows.
- **Nearing's three JPEG captures are cropped from the App Store screenshots** to remove the marketing headline baked into each ("Run as many as you need", "11 colours. Two sizes"). On the store that headline is the point; on a page it sits under the section's own `h2` and competes with it — in one case it contradicted it. If you re-export from the store versions, crop them again.
